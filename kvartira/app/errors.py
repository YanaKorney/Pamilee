...
def ai_not_configured(what: str) -> UserError:
    return UserError(
        f"Сервис «{what}» не настроен: нет ключа доступа.",
        "Откройте «Настройки» — ссылка справа вверху — и вставьте ключ "
        "в поле «Новый ключ». Искать файлы и перезапускать программу не нужно.",
    )
...
def database_broken(path: "object") -> UserError:
    return UserError(
        "Файл с вашими проектами повреждён и не открывается.",
        "Откройте папку с данными, переименуйте файл data/app.db "
        "в app-старый.db и запустите программу заново — она создаст новый. "
        "Загруженные планы и картинки при этом никуда не денутся, "
        "но список проектов придётся собрать заново.",
        technical=f"повреждён файл {path}",
    )


def key_has_strange_characters() -> UserError:
    return UserError(
        "В ключе доступа есть посторонние символы.",
        "Похоже, при копировании захватилось что-то лишнее. Скопируйте ключ "
        "из личного кабинета ещё раз — только сам ключ, без пробелов по краям "
        "и без кавычек.",
    )
