...
class OpenAiCompatProvider:
    ...
    def _explain(self, response: httpx.Response) -> UserError:
        ...
        if code == 402 or "insufficient" in detail.lower() or "баланс" in detail.lower():
            return UserError(
                "На счету не хватает денег на этот запрос.",
                "Пополните баланс в личном кабинете сервиса. Учтите: сервис "
                "резервирует сумму не по факту, а по потолку выбранной модели — "
                "поэтому дорогая модель «съедает» остаток даже за короткий "
                "ответ. Модель подешевле выбирается в «Настройках».",
                technical=note,
            )
        if code == 404:
            return UserError(
